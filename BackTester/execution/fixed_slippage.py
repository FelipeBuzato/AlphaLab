...
class FixedSlippageExecutor(OrderExecutor):
    def __init__(self, transaction_cost_rate, slippage_rate):
        super().__init__()
    ...
    def execute_orders(self, date, prices_open, current_shares, delta_shares, cash):
        ...
        for ticker in sell_shares[sell_shares > 0].index:
            execution_sell_price = market_open_prices[ticker] * (1 - self.slippage_rate)
        ...
        for ticker in buy_shares[buy_shares > 0].index:
            execution_buy_price = market_open_prices[ticker] * (1 + self.slippage_rate)
            trade_value = buy_shares[ticker] * execution_buy_price
            transaction_cost = trade_value * self.transaction_cost_rate
            cash_reduction = trade_value + transaction_cost

            # if transaction costs + slippage result in the need of more cash than we actually
            # have, compute the maximum number of shares that can be bought 
            if(cash < cash_reduction):
                adjusted_buy_shares = self.adjust_buy_order(cash, market_open_prices[ticker], self.transaction_cost_rate, self.slippage_rate)
    ...
    def adjust_buy_order(self, cash, market_open_price, transaction_cost_rate, slippage_rate):
        adjusted_shares = cash / (market_open_price * (1 + transaction_cost_rate) * (1 + slippage_rate))
